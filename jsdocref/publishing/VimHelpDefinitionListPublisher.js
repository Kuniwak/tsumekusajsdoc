...
jsdocref.addSingletonGetter(VimHelpDefinitionListPublisher);
 ...
 * @param {jsdocref.publishing.DefinitionList.ListType} listType List type.
 ...
 * @param {jsdocref.publishing.DefinitionList} list List.
...
VimHelpDefinitionListPublisher.prototype.publish = function(list) {
  ...
  var output = list.getDefinitions().map(function(def, index) {
    ...
    result.push(wrapper.wrap(caption, jsdocref.TEXT_WIDTH, indentLevel));
    result.push(wrapper.wrap(content, jsdocref.TEXT_WIDTH, markerWidth +
                           indentLevel));
    return result.join('\n');
  }, this);
